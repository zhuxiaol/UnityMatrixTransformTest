...
Shader "custom/Shuziyu"
{
    Properties
    {
        ...
        _CellSize ("格子数量，只用xy", Vector) = (10, 10, 0.03, 0)
    ...
    Subshader
    {
        ...
        Pass
        {
            ...
            fixed4 frag2 (v2f i) : COLOR
            {
				...
				i.uv +=speedUv;
				//float2 randomUv = float2( floor(i.uv.x*_CellSize.x)*1/_CellSize.x,floor(i.uv.y*_CellSize.y)*1/_CellSize.y);
				float2 randomUv = float2( floor(i.uv.x*25)*1/25,floor(i.uv.y*25)*1/25);
				float2 flowUv=randomUv;
				randomUv+=speedUv*0.08;
				float random= tex2D(_RandomTex,randomUv).r;
				float off= round(random*_TexelSizes.z)*1/_TexelSizes.z;
				float2 tmpUv =float2(frac( i.uv.x*_TexelSizes.z)*1/_TexelSizes.z+0,i.uv.y*_CellSize.y);
				//tmpUv+=speedUv;
